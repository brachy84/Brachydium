package brachydium.lib.unification;

import brachydium.lib.unification.material.MarkerMaterial;
import brachydium.lib.unification.material.Material;
import brachydium.lib.unification.material.properties.PropertyKey;
import brachydium.lib.unification.ore.OrePrefix;
import brachydium.lib.unification.stack.ItemMaterialInfo;
import brachydium.lib.unification.stack.MaterialStack;
import brachydium.lib.unification.stack.UnificationEntry;
import brachydium.lib.util.ItemAndMeta;
import brachydium.lib.util.ItemMaps;
...
public class OreDictUnifier {
    ...
    private static final ItemMaps.ItemAndMetaMap<ItemMaterialInfo> materialUnificationInfo = new ItemMaps.ItemAndMetaMap<>(true);
    private static final ItemMaps.ItemAndMetaMap<UnificationEntry> stackUnificationInfo = new ItemMaps.ItemAndMetaMap<>(true);
    private static final Map<UnificationEntry, ArrayList<ItemAndMeta>> stackUnificationItems = new Object2ObjectOpenHashMap<>();
    private static final ItemMaps.ItemAndMetaMap<Set<String>> stackOreDictName = new ItemMaps.ItemAndMetaMap<>(true);
    private static final Map<String, List<ItemStack>> oreDictNameStacks = new Object2ObjectOpenHashMap<>();

    @Nullable
    private static Comparator<ItemAndMeta> stackComparator;

    public static Comparator<ItemAndMeta> getSimpleItemStackComparator() {
        if (stackComparator == null) {
            List<String> modPriorities = Collections.emptyList();//TODO Arrays.asList(ConfigHolder.compat.modPriorities);
            if (modPriorities.isEmpty()) {
                //noinspection ConstantConditions
                Function<ItemAndMeta, String> modIdExtractor = stack -> stack.getItem().getRegistryName().getNamespace();
    ...
    public static Comparator<ItemStack> getItemStackComparator() {
        Comparator<ItemAndMeta> comparator = getSimpleItemStackComparator();
        return (first, second) -> comparator.compare(new ItemAndMeta(first), new ItemAndMeta(second));
    }
    ...
    public static void registerOre(ItemStack itemStack, ItemMaterialInfo materialInfo) {
        ...
        materialUnificationInfo.put(new ItemAndMeta(itemStack), materialInfo);
    }
    ...
    public static void onItemRegistration(OreRegisterEvent event) {
        ItemAndMeta simpleItemStack = new ItemAndMeta(event.getOre());
        ...
        if (orePrefix != null && (material != null || orePrefix.isSelfReferencing)) {
            ...
            ArrayList<ItemAndMeta> itemListForUnifiedEntry = stackUnificationItems.computeIfAbsent(unificationEntry, p -> new ArrayList<>());
    ...
    public static Set<String> getOreDictionaryNames(ItemStack itemStack) {
        ...
        ItemAndMeta simpleItemStack = new ItemAndMeta(itemStack);
    ...
    public static UnificationEntry getUnificationEntry(ItemStack itemStack) {
        if (itemStack.isEmpty()) return null;
        return stackUnificationInfo.get(new ItemAndMeta(itemStack));
    }
    ...
    public static ItemStack getUnificated(ItemStack itemStack) {
        ...
        if (unificationEntry == null || !stackUnificationItems.containsKey(unificationEntry) || !unificationEntry.orePrefix.isUnificationEnabled)
            return itemStack;
        ArrayList<ItemAndMeta> keys = stackUnificationItems.get(unificationEntry);
    ...
    public static List<ItemStack> getAll(UnificationEntry unificationEntry) {
        ...
        ArrayList<ItemAndMeta> keys = stackUnificationItems.get(unificationEntry);
        return keys.stream().map(ItemAndMeta::toItemStack).collect(Collectors.toList());
    }
